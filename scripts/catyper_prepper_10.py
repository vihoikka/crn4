...
# length_lim arg
parser.add_argument(
    "-ll", "--no_length_lim", help="Don't include length limit for RNs", required=False
)
...
if mode == "post_hmm":
    ...
    # if --length_lim is used as argument, create a dictionary with length limits at 9999 for all effectors
    if args.no_length_lim == "True":
        print("'No length limit' argument used")
        for key, value in max_lengths.items():
            max_lengths[key] = 9999
        print("New length max lim table")
        print(max_lengths)
